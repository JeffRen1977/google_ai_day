...
（与 day1、day2、day3、day4 共享相同的模型工具函数）
"""
...
def get_model_for_task(task_type: str = "general") -> str:
    ...
    Args:
        task_type: 任务类型 ("simple", "complex", "general", "fast")
        
    Returns:
        模型名称
    """
    if task_type == "simple" or task_type == "fast":
        # 简单任务使用更快的 Flash 模型
        return get_model_name(['gemini-1.5-flash', 'gemini-1.5-pro'])
    elif task_type == "complex":
        # 复杂任务使用更强大的 Pro 模型
        return get_model_name(['gemini-1.5-pro', 'gemini-1.5-flash'])
    else:
        # 默认使用 Flash 模型（更快、更经济）
        return get_model_name(['gemini-1.5-flash', 'gemini-1.5-pro'])
...
if __name__ == "__main__":
    ...
    print(f"\n简单任务推荐模型: {get_model_for_task('simple')}")
    print(f"复杂任务推荐模型: {get_model_for_task('complex')}")
